...
void insert_transitions(int pos, automaton& a, transition* tr, int tr_size) {
	if(pos >= a.table_size)
		expand_table(a);
	for (int i = 0; i < tr_size; i++) {
		a.table[i].ch = tr[i].ch;
		a.table[i].source = tr[i].source;
		a.table[i].dest = tr[i].dest;
		a.table[i].offset = pos;
	}
}
